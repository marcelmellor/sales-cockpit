...
export const LEADS_CACHE_TTL_SECONDS = 5 * 60;

export function leadsCacheKey(produkt: string | null): string {
  return `leads-overview:${produkt ?? '_all'}`;
}

export class LeadPipelineNotFoundError extends Error {
...
export async function buildLeadsOverview(produkt: string | null): Promise<LeadsOverviewResponse> {
